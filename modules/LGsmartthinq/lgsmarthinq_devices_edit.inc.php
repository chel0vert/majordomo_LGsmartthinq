...
$table_name = 'lgsmarthinq_devices';
...
if ($this->tab == 'data') {
    ...
    if ($this->mode == 'update') {
        ...
        if ($title_new) {
            ...
            $new_id = SQLInsert('lgsmarthinq_values', $prop);
        }
    }
    global $delete_id;
    if ($delete_id) {
        SQLExec("DELETE FROM lgsmarthinq_values WHERE ID='" . (int)$delete_id . "'");
    }
    $properties = SQLSelect("SELECT * FROM lgsmarthinq_values WHERE DEVICE_ID='" . $rec['ID'] . "' ORDER BY ID");
    ...
    for ($i = 0; $i < $total; $i++) {
        ...
        if ($this->mode == 'update') {
            ...
            SQLUpdate('lgsmarthinq_values', $properties[$i]);
